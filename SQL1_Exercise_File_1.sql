...
select length(geo_location) from bank_inventory where geo_location = 'delhi_city';
...
update bank_inventory set quantity=10 where product = 'paycard' and quantity = 2;
 # Question 12:
...
insert into bank_holidays values (now(), now(), now() );
